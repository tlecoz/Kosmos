package kosmos.utils.box2dUtils.earCutting;

import org.jbox2d.common.Vec2;

//the code come from here : https://github.com/libgdx/libgdx/blob/master/gdx/src/com/badlogic/gdx/math/EarClippingTriangulator.java
...
public class EarCutting {
	...
	static private EarCutting instance;
	
	public EarCutting(){
		instance = this;
	}
	
	static public EarCutting getInstance(){
		if(instance == null) new EarCutting();
		return instance;
	}
	...
	/** @see #computeTriangles(float[], int, int) */
	public ShortArray computeTriangles (Vec2[] vertices) {
		float[] verts = new float[vertices.length*2];
		int i,j=0,len = vertices.length;
		for(i=0;i<len;i++){
			verts[j++] = vertices[i].x;
			verts[j++] = vertices[i].y;
		}
		return computeTriangles(verts, 0, verts.length);
	}
